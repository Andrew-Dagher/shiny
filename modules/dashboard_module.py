...
_METRICS = ["Quotes","Sales","Written_Premiums","Closing_Ratio","Avg_Premium","Avg_CLV"]
METRIC_LABELS = {
    "Quotes": "Quotes","Sales": "Sales","Written_Premiums": "Written Premiums",
    "Closing_Ratio": "Closing Ratio","Avg_Premium": "Average Premium","Avg_CLV": "Average CLV",
}
COLORS = {
    "Quotes":"tab:blue","Sales":"tab:orange","Written_Premiums":"tab:green",
    "Avg_Premium":"tab:purple","Avg_CLV":"tab:brown","Closing_Ratio":"tab:green",
}
SCALE = {
    "Quotes":1,"Sales":1,"Written_Premiums":1/1000,
    "Closing_Ratio":1,"Avg_Premium":1,"Avg_CLV":1/1000,
}
        ...
        ui.layout_column_wrap(
            ...
            ui.card(
                ...
                ui.input_checkbox_group(
                    ...
                    choices={k:v for k,v in METRIC_LABELS.items()
...
def dashboard_server(input, output, session, filtered_data):
    ...
    def kpi_wide() -> pd.DataFrame:
        df = filtered_data()
        # build the same metric → phone → web rows per category
        if input.show_top30():
        ...
        for m in _METRICS:
            for prefix, chan in [(f"<b>{METRIC_LABELS[m]}</b>", None),
                ...
                for cat_name, sub in cats:
                    ...
                    s = sub.sort_values("Month")[m]
                    num = s.mean() if m=="Closing_Ratio" else s.sum()
                    pct = 0 if len(s)<2 or s.iloc[0]==0 else (s.iloc[-1]-s.iloc[0])/s.iloc[0]*100
        ...
        cols = ["Metrics"] + [c for cat,_ in cats for c in (f"{cat} {last_lbl}",f"{cat} YoY")]
        return wide[cols]
...
"""
    ...
    def sales_closing_plot():
        ...
        x = range(len(df)); bottom=[0]*len(df)
        fig, ax = plt.subplots(figsize=(10,6))
        for m in ("Quotes","Sales"):
            vals = (df[m]*SCALE[m]).tolist()
            ax.bar(x, vals, bottom=bottom, width=0.6, alpha=0.5,
        ...
        ax.set_xlabel("Month"); ax.set_ylabel("Value"); ax.set_title("Sales & Closing Trends")
    ...
    def other_trends_plot():
        df = filtered_data().sort_values("Month"); sel=input.other_metrics()
        x = range(len(df))
        fig, ax = plt.subplots(figsize=(10,6))
        for m in sel:
            ax.plot(x, df[m]*SCALE[m], marker="o", color=COLORS[m], label=METRIC_LABELS[m])
        ax.set_xticks(x)
        ax.set_xticklabels(df["Month"].dt.strftime("%b %Y"), rotation=45)
        ax.set_xlabel("Month"); ax.set_ylabel("Value"); ax.set_title("Other Metrics Trends")
        ax.legend(loc="upper left"); fig.tight_layout(); return fig
    ...
    def other_trend_indicators():
        ...
        for m in sel:
            ...
            items.append(
                ui.div(ui.h5(METRIC_LABELS[m]),
                       ui.p(f"{arrow} {pct:+.1f}%", style=f"color:{color};font-weight:bold;"),
                       style="margin-right:15px;margin-bottom:10px;")
            )
        return ui.div(ui.h4("Trend Analysis (Month-over-Month)"),
